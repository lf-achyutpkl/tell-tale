...
@Setter
@NamedQueries({
    @NamedQuery(name = DecryptionKey.GET_PRIVATE_KEY, query = "SELECT s FROM Session s WHERE s.user = :user") })
public class Session extends AbstractEntity implements Serializable {
    ...
    private static final String PREFIX = "tell.tale.decryption.session";
    public static final String GET_PRIVATE_KEY = PREFIX + "getDecryptionKey";
